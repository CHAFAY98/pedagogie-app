...
function HomeComponent() {
  ...
  const allMatieresRef = useRef<Matiere[]>([]);
  const searchMatiere=useRef(null);
  ...
  const specialiteTap = (id: number) => {
    ...
    const filteredMatieres = newMatieres.filter(
      (matiere) => matiere.id_specialite === id
    );
    setMatieres(filteredMatieres);
    allMatieresRef.current=filteredMatieres;
  };
  const stage_tabs =
    stages.length > 0 ? (
      stages.map((stg) => (
  ...
  const specialites_tab =
    ...
    const handleChangeMatiere = () => {
      const searchValue = searchMatiere?.current?.value.toLowerCase();
      if (!searchValue) {
        setMatieres(allMatieresRef.current);
        return;
      }
      const filteredMatieres = allMatieresRef.current.filter((matiere) =>
        matiere.libelle.toLowerCase().includes(searchValue)
      );
      setMatieres(filteredMatieres);
    };
